...
class Capitals(StatesGroup):
    """Класс с капиталами и репортами"""
...
async def send_welcome(message):
    """Отправка краткого описания бота и
    потенциальное ознакомление с его функциями
    """
...
async def send_error(message, state="*"):
    ...
    которое бот не сможет распознать, отправится просьба
