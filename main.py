...
async def send_welcome(message: types.Message):
    await message.answer("Здравствуйте!!!\nЯ Северствль бот\nЧтобы ознакомиться с моими функциями на клавиатуре "
                         "нажмите на кнопку 'Помощь'", reply_markup=kb.markup1)
...
@dp.message_handler(lambda message: message.text in ['Общий доход компании', 'Акционерный капитал', 'Чистая прибыль'], state=capital_or_report.choice)
...
@dp.message_handler(lambda message: message.text in ['Квартальный отчёт', 'Годовой отчёт'], state=capital_or_report.period)
async def send_report(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        data['period'] = message.text
    await message.answer(report[data['choice']][data['period']], reply_markup=kb.markupF)
...
async def send_structure(message: types.Message):
    await message.answer(ac.severstal_russian_steel, reply_markup=kb.markup_about_company)
...
async def send_further_redistribution_enterprises(message: types.Message):
    await message.answer(ac.further_redistribution_enterprises, reply_markup=kb.markup_about_company)
...
async def send_leadership(message: types.Message):
    await message.answer('Выберите тип руководства', reply_markup=kb.markup_of_leadership)


@dp.message_handler(TextFilter(equals=kb.management_of_severstal_management_JSC), state='*')
async def send_management_of_severstal_management_JSC(message: types.Message):
    await message.answer(ac.management_of_severstal_management_JSC, reply_markup=kb.markup_about_company)
...
@dp.message_handler(lambda message: message.text in ['Производство', 'IT & Digital', 'Офис', 'Молодым специалистам'], state=vaca.vacation)
...
async def send_vactions(message: types.Message, state: FSMContext):
    ...
    await message.answer(direction[data['vacation']][data['city']], reply_markup=kb.markup1)
